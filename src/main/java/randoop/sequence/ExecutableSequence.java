...
public class ExecutableSequence {
  ...
  public List<Object> getLastStmtObjectsResLast() {
	  List<Object> res = new ArrayList<>();
	  int last = sequence.size() -1;
	  Object[] objsAfterExec = getRuntimeInputs(last);
	  for (int j = 0; j < objsAfterExec.length; j++) {
  ...
  public List<Object> getLastStmtActiveObjects(String className) {
		List<Object> res = new LinkedList<>();
		if (!genNewObjects()) return res;
		
		List<Object> lastStmtObjs = getLastStmtObjects();
  ...
  public boolean exceedsScopes = false;
