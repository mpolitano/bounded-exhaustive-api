    ...
    for (TypeVariable typeArgument : parameters) {
      ...
      if (GenInputsAbstract.instance_generics_integer) {
    	  ...
    	  if (selectedTypes.isEmpty()) {
			  for (int k = 0; k < candidates.size(); k++) {
				  ReferenceType t = candidates.get(k);
				  if (/*t.isBoxedPrimitive() &&*/ t.toString().equals("java.lang.String")) {
					  selectedTypes.add(t);
					  break;
				  }
			  }
	    	  ...
	    	  if (selectedTypes.isEmpty()) {
	        	  for (int k = 0; k < candidates.size(); k++) {
	        		  ReferenceType t = candidates.get(k);
	        		  if (t.isBoxedPrimitive() && t.toString().equals("java.lang.Long")) {
	        			  selectedTypes.add(t);
	        			  break;
	        		  }
	        	  }
				  ...
				  if (selectedTypes.isEmpty()) {
					  selectedTypes.add(Randomness.randomMember(candidates));
	//    			  throw new Error("Could not instance generic type parameter with java.lang.Integer or java.lang.String");
				  }
        	  }
    	  }
      }
